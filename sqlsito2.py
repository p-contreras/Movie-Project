...
def filter(cnx, mdict, description):
	cur = cnx.cursor(buffered = True)
	f_list = []
	
	if description == "Movie":
		for m in mdict["Movie List"]:
			cur.callproc("Check_Movie_Title", [m])
			for result in cur.stored_results():
				val = result.fetchall()
				if val != []:
					f_list.append(val[0][0])
		mdic["Movie List"] = f_list

	elif description == "People":
		for p in mdict["People"]:
			cur.callproc("Check_People", [p])
			for result in cur.stored_results():
				val = result.fetchall()
				if val != []:
					f_list.append(val[0][0])
		...
		for t in mdict["Movie List"]:
			cur.callproc("Check_TV_Title", [t])
			for result in cur.stored_results():
				val = result.fetchall()
				if val != []:
					f_list.append(val[0][0])
